...
from revoscalepy import rx_import_datasource
...
def main(tablename, inputdf, overwrite=False):
    ...
    if overwrite:
        ...
        rx_import_datasource(input_data=inputdf, \
            output_file=outfile, overwrite=True)
    ...
    data_source_train = RxSqlServerData(
        ...
        column_info={
            ...
            "Annual_Income_Bucket_gt120k" : {"type" : "factor"},
            "Highest_Education_Graduate_School" : {"type" : "factor"}
            }
        )
    ...
    # # NB: If this is regression (rx_lin_mod) can look at metrics like:
...
if __name__ == '__main__':
    ...
    xdf_source = RxTextData(file=inputfile, delimiter=',')

    xdf = rx_import_datasource(input_data=xdf_source, output_file=output_file,
                                overwrite=True)

    rx_data_step(input_data=xdf, output_file=xdf, 
